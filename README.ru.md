...
Интерактивный bash-скрипт, который ускоряет выдачу нужных разрешений агентам OpenClaw — без открытия Dashboard и без предоставления полного доступа.
...
- Агент блокируется на безобидных командах (`cat`, `ls`, `grep`) и каждую нужно подтверждать через Dashboard
- Dashboard UI не работает удалённо (например, через Telegram) — каждый раз нужно открывать браузер
- Единственная альтернатива — `security: "full"`, то есть полный доступ без контроля
- Редиректы (`2>/dev/null`, `2>&1`) и пайпы (`|`) блокируются в режиме allowlist даже когда все бинарники разрешены
- Allowlist'ы — per-agent без наследования: `agents["*"]` не покрывает `agents["main"]`

Этот скрипт выставляет **рекомендованный baseline** одной командой: режим `allowlist` + выбранные группы разрешений + правила AGENTS.md + чистое наследование настроек агентов. Он также обходит [известные проблемы OpenClaw](#известные-проблемы), которые вызывают промпты даже при правильной конфигурации.
...
| 7 | Обновляет AGENTS.md с правилами Shell Command Rules | Предотвращает проблемы с редиректами/пайпами |
